...
**Logic**: `and` (a macro), `or`, and `not` (operatives). `and` and `or` short-circuit, and treat every operand, including the last, as a Church Boolean, so a non-Boolean operand is an error. `nand` and `nor` are macros: `(nand x ...)` is `(not (and x ...))` and `(nor x ...)` is `(not (or x ...))`, so they short-circuit the same way. `xor` and `xnor` are functions, which evaluate every operand: `xor` is true when an odd number of its operands are true, so `(xor a b c)` is `(xor (xor a b) c)`, and `(xnor ...)` is `(not (xor ...))`. `(xor)` is false and `(xnor)` is true.
